...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{../figuras/qubit_portas_probabilidades.png}
    \caption{Representação visual de um qubit em superposição, circuitos quânticos simples e distribuições de probabilidade após a aplicação de portas como \(H\), \(X\), \(R_Y\) e CNOT. A medição transforma o estado quântico em resultados clássicos probabilísticos. Fonte: elaborado pelo autor com base na documentação da IBM Quantum sobre circuitos e medição \cite{ibmQuantumCircuitLibrary2026,ibmQuantumMeasureQubits2026}.}
    \label{fig:qubit_portas_probabilidades}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{../figuras/codificacoes_quanticas_imagem.png}
    \caption{Comparação esquemática entre três representações quânticas de imagens: Qubit Lattice, FRQI e NEQR. A figura destaca diferentes formas de associar posição e intensidade dos pixels a estados ou registradores quânticos. Fonte: elaborado pelo autor com base em Okada et al. \cite{okada2025measurementlimits}.}
    \label{fig:codificacoes_quanticas_imagem}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{../figuras/circuito_variacional_exemplo.png}
    \caption{Fluxo simplificado de um circuito variacional: os dados são codificados no circuito, o ansatz parametrizado transforma o estado, as medições produzem valores clássicos e um otimizador ajusta os parâmetros. Fonte: elaborado pelo autor com base em Cerezo et al. \cite{cerezo2021variational} e na documentação do circuito \textit{real amplitudes} do Qiskit \cite{ibmQuantumRealAmplitudes2026}.}
    \label{fig:circuito_variacional_exemplo}
\end{figure}
...
O Qiskit será utilizado como ferramenta de apoio para a construção e simulação dos circuitos quânticos variacionais. Em especial, simuladores como o Qiskit Aer são relevantes para estudos experimentais porque permitem executar circuitos com controle sobre número de \textit{shots}, medições, estados simulados e custo computacional \cite{faj2023warpspeed}. Estudos recentes também discutem fluxos híbridos quântico-clássicos e aceleração por GPU em simulações quânticas, incluindo o uso de ferramentas como cuQuantum \cite{chen2024quantumhpcworkflow,bayraktar2023cuquantum}. Ainda assim, os resultados obtidos por simulação devem ser interpretados com cautela, pois podem não refletir integralmente ruído, conectividade, calibração e restrições operacionais de dispositivos NISQ reais.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{../figuras/simulacao_qiskit_fluxo.png}
    \caption{Fluxo simplificado de simulação de circuitos quânticos com Qiskit/Aer. O circuito é executado em um backend simulado, produzindo estados ou contagens de medição; o custo cresce com o número de qubits, profundidade e número de execuções. Fonte: elaborado pelo autor com base em Faj et al., Chen et al. e Bayraktar et al. \cite{faj2023warpspeed,chen2024quantumhpcworkflow,bayraktar2023cuquantum}.}
    \label{fig:simulacao_qiskit_fluxo}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{../figuras/modelo_hibrido_quantico_classico.png}
    \caption{Esquema geral de uma arquitetura híbrida quântico-clássica para classificação de mamografias. A CNN extrai características, uma projeção reduz a dimensão, o circuito quântico processa a representação compacta e uma camada clássica final produz as probabilidades das classes. Fonte: elaborado pelo autor com base em propostas de modelos híbridos quântico-clássicos \cite{long2025qcqcnn,schetakis2022qmlframeworks}.}
    \label{fig:modelo_hibrido_quantico_classico}
\end{figure}
